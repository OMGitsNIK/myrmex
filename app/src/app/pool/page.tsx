    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full inline-block ${
          report.is_fresh ? "bg-[var(--accent)]" : "bg-yellow-500"
        }`}
      />
      <span
        className={report.is_fresh ? "text-[var(--accent)]" : "text-yellow-500"}
      >
...
export default function PoolPage() {
  ...
  const refreshLpBalance = useCallback(
    async (poolPubkey: string, lpMint: PublicKey) => {
      ...
      const providerLpAta = getAssociatedTokenAddressSync(
        lpMint,
        wallet.publicKey
      );
      try {
  ...
  const handleDeposit = async (poolPubkey: string) => {
    ...
    if (amount > 1_000_000) {
      toast.error("Deposit exceeds maximum (1,000,000 USDC)");
      return;
    }
    ...
    try {
      ...
      const poolName =
        COVERAGE_NAMES[poolAccount.poolType] ??
        `Pool Type ${poolAccount.poolType}`;

      const lpMintKey = new PublicKey(lpMint);
      const providerUsdc = getAssociatedTokenAddressSync(
        USDC_MINT,
        wallet.publicKey
      );
      const providerLpTokens = getAssociatedTokenAddressSync(
        lpMintKey,
        wallet.publicKey
      );
  ...
  const handleWithdraw = async (poolPubkey: string) => {
    ...
    if (maxLp > 0 && amount > maxLp) {
      toast.error(`Insufficient LP balance (max ${maxLp.toFixed(4)})`);
      return;
    }
            ...
            >
              <div
                className={`absolute top-0 right-0 w-32 h-32 ${
                  s.kind === "deposit" ? "bg-[var(--accent)]" : "bg-blue-500"
                } opacity-5 rounded-full blur-2xl pointer-events-none`}
              />
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <div className="text-sm text-gray-400">
                    {s.poolName} Pool · {s.timestamp.toLocaleTimeString()}
                  </div>
                </div>
                <div className="flex flex-col sm:items-end gap-2">
                  <div className="flex items-center gap-2">
                    <div className="text-xs text-gray-500 font-mono">
                      {s.txSig.slice(0, 8)}...{s.txSig.slice(-6)}
                    </div>
                  ...
                  <a
                    ...
                    className={`text-xs border px-3 py-1.5 rounded transition-colors ${
                      s.kind === "deposit"
                        ? "border-[var(--accent)]/40 text-[var(--accent)] hover:border-[var(--accent)]"
                ...
                {s.kind === "deposit" ? (
                  <>
                    ...
                    <div>
                      ...
                      <div className="text-[var(--accent)] font-medium font-mono">
                        Check wallet
                      </div>
                    </div>
                    <div>
              ...
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <div className="font-semibold text-white text-lg">
                    {COVERAGE_NAMES[p.poolType] ?? `Pool Type ${p.poolType}`}{" "}
                    Pool
                  </div>
                  <div className="text-xs text-gray-500 font-mono">
                ...
                <div className="flex justify-between text-xs text-gray-500">
                  ...
                  {wallet && (
                    <span className="font-mono">
                      Balance:{" "}
                      {(lpBalances[poolKey] ?? 0).toLocaleString(undefined, {
