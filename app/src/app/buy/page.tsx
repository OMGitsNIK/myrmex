...
export default function BuyPage() {
  ...
  const quoteParams = {
    ...selectedType.pricingParams(threshold, payoutAmount, durationDays, extraField(
      (selectedType.extraFields as readonly any[])[0]?.key ?? ""
    )),
  ...
  const riskColor =
    !quote ? "text-gray-400"
    : quote.risk_score < 20 ? "text-[var(--accent)]"
    : quote.risk_score < 50 ? "text-yellow-400"
    : "text-red-400";
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      const matchingPool = allPools.find((p: any) => p.poolType === selectedType.id && p.isActive);
      if (!matchingPool) {
      ...
      const [policyPda] = PublicKey.findProgramAddressSync(
        [Buffer.from("policy"), wallet.publicKey.toBuffer(), poolPda.toBuffer(), Buffer.from(nonce.toArray("le", 8))],
        programId
      );
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      {policies.length > 0 && (
        <div className="space-y-4">
          ...
          {policies.map((pol, i) => (
            <div key={i} className="relative card p-6 border-[var(--accent)]/30 overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                ...
                <div>
                  ...
                  <div className="text-[var(--accent)] font-bold text-base">${pol.payoutAmount.toLocaleString()} USDC</div>
                </div>
                <div>
                  <div className="text-gray-500 text-xs mb-0.5">Premium Paid</div>
                  <div className="text-white">${pol.premiumPaid.toFixed(2)} USDC</div>
                </div>
                <div>
                  <div className="text-gray-500 text-xs mb-0.5">Expires</div>
                  <div className="text-white">{pol.expiresAt.toLocaleDateString()}</div>
                </div>
                <div>
                  <div className="text-gray-500 text-xs mb-0.5">Trigger</div>
                  <div className="text-white font-mono">{pol.thresholdDisplay}</div>
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-[var(--border)] flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono text-gray-500">{pol.policyKey.slice(0, 12)}…{pol.policyKey.slice(-6)}</span>
                  <button
                    onClick={() => { navigator.clipboard.writeText(pol.policyKey); setCopiedKey(pol.policyKey); setTimeout(() => setCopiedKey(null), 1500); }}
      ...
      <div className="grid grid-cols-3 gap-3">
        {COVERAGE_TYPES.map((ct) => (
          ...
          >
            ...
            <div className="font-medium text-white text-sm leading-tight">{ct.name}</div>
            <div className="text-[10px] text-gray-500 mt-1 font-mono">{ct.marketGap} gap</div>
          </button>
        ))}
      </div>

      {/* Selected category detail */}
      <div className="card p-5 space-y-1 border-l-2" style={{ borderLeftColor: selectedType.color }}>
        <p className="text-white font-semibold">{selectedType.name}</p>
        <p className="text-gray-400 text-sm">{selectedType.description}</p>
        <p className="text-xs text-gray-600 pt-1">Oracle: {selectedType.oracleSource}</p>
      </div>
        ...
        {quote && !quoteLoading && (
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`font-mono font-bold ${riskColor}`}>{quote.risk_score.toFixed(1)}/100</span>
            </div>
            <div className="flex justify-between items-center">
